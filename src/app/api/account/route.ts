...
export async function PATCH(req: NextRequest) {
  if (!allow("account-edit", req, 20)) return NextResponse.json({ error: "rate_limited" }, { status: 429 });

  const su = await getSessionUser();
  if (!su || !su.accountId) return NextResponse.json({ error: "Sign in to edit your profile." }, { status: 401 });
