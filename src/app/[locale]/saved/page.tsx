...
export default function SavedPage({ params }: { params: { locale: string } }) {
  ...
  const pxNote = (id: string) => {
    ...
    return (
      <span className="fig block text-[11px] font-semibold" style={{ color: down ? "#1B7A50" : "#8A5A1F" }}>
            ...
            <div className="mt-3 overflow-x-auto rounded-2xl border border-line bg-white">
              <table className="w-full min-w-[640px] text-sm">
                ...
                <tbody className="text-charcoal/75">
                  ...
                  <Row label={T.vsIdx}>{shownL.map((l) => { const v = (l as any).vs_index; if (!v) return <Cell key={l.id}><span className="text-charcoal/40">{T.noIndex}</span></Cell>; const a = Math.abs(v.deltaPct ?? 0); const txt = v.status === "below" ? `${T.belowPre}${a}${T.belowSuf}` : v.status === "above" ? `${T.abovePre}${a}${T.aboveSuf}` : T.withinBand; const col = v.status === "below" ? "#1B7A50" : v.status === "above" ? "#8A5A1F" : "var(--harbor)"; return <Cell key={l.id}><span className="fig" style={{ color: col, fontWeight: 600 }}>{txt}</span></Cell>; })}</Row>
