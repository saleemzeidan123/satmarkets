...
export default async function DashboardPage({ params }: { params: { locale: string } }) {
  ...
  if (me?.account_id) {
    const { data } = await sb.from("listings").select("*").eq("account_id", me.account_id).order("created_at", { ascending: false });
    listings = (data as Listing[]) ?? [];
  }
  const acct: any = me?.accounts;
  const verified = acct?.verification_status === "verified";
    ...
    <section className="py-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3">
          {isSat && <Link href={`/${locale}/admin`} className="rounded border border-charcoal/20 px-3 py-1.5 text-sm">Admin</Link>}
          {me?.account_id && <Link href={`/${locale}/dashboard/new`} className="rounded bg-gold px-3 py-1.5 text-sm text-white">New listing</Link>}
        </div>
      </div>
      <p className="mt-2 text-sm text-charcoal/60">Signed in as {auth.user.email}{acct?.legal_name ? ` · ${acct.legal_name}` : ""}</p>
      {me?.account_id && !verified && (
        <div className="mt-4 rounded border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-800">{dict.dash.verifyPending}</div>
      )}
      <div className="mt-6 overflow-hidden rounded-lg border border-charcoal/10 bg-white">
        <table className="w-full text-sm">
          <thead className="bg-ivory text-left text-xs uppercase text-charcoal/50">
            <tr><th className="px-4 py-2">Title</th><th className="px-4 py-2">Type</th><th className="px-4 py-2">Area</th><th className="px-4 py-2">Status</th></tr>
          </thead>
          <tbody>
            {listings.length === 0 ? (
              ...
              <tr key={l.id} className="border-t border-charcoal/5">
                ...
                <td className="px-4 py-2"><span className="rounded bg-slate/10 px-2 py-0.5 text-xs text-slate">{l.status}</span></td>
              </tr>
            ))}
          </tbody>
        </table>
