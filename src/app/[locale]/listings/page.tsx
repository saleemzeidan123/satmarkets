  ...
  return (
    <div style={{ maxWidth: 1360, margin: "0 auto", padding: "28px 24px 64px", fontFamily: "var(--sans)", color: "var(--ink)" }}>
      ...
      <div className="row between wrap" style={{ marginTop: 14, alignItems: "center", gap: 10 }}>
        ...
        <div role="status" aria-live="polite" className="muted" style={{ fontSize: "0.8125rem" }}>{dataOk ? formatCounted(shown.length, "space", locale) : dl.countUnavailable}{dataOk && searchParams.place && (!placeIds || !placeIds.size) ? " · " + fill(dl.noSpacesIn, { place: searchParams.place }) : ""}{dataOk && bbox ? <> {"AAA\\u00B7BBB"} {dl.mapArea} {"AAA\\u00B7BBB"} <Link href={`/${locale}/listings?${base}`} style={{ color: "var(--harbor)", textDecoration: "none", fontWeight: 600 }}>{dl.clearArea}</Link></> : null}{dataOk && capped ? <> {"AAA\\u00B7BBB"} {fill(dl.capNote, { n: formatNumber(listings.length, locale) })}</> : null}</div>
