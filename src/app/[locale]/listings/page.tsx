...
export default async function ListingsPage({ params, searchParams }: { params: { locale: string }; searchParams: { asset?: string; deal?: string; q?: string } }) {
  ...
  return (
    <div style={{ maxWidth: 1360, margin: "0 auto", padding: "28px 24px 64px", fontFamily: "var(--sans)", color: "var(--ink)" }}>
      ...
      <div className="muted" style={{ marginTop: 14, fontSize: 13 }}>{ar ? `${listings.length} عرض موثّق` : `${listings.length} verified ${listings.length === 1 ? "space" : "spaces"}`}</div>
      {listings.length === 0 ? (
        <p className="muted" style={{ marginTop: 28 }}>{ar ? "لا توجد مساحات مطابقة. جرّب توسيع عوامل التصفية." : "No matching spaces. Try widening your filters."}</p>
      ) : (
