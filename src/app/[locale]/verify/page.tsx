          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} style={{ background: needs(r) ? "#FBF3E6" : "var(--paper)" }}>
                <td style={td}><span style={{ fontFamily: "var(--font-mono), monospace", fontSize: "0.6875rem" }}>{r.reference_code || r.id.slice(0, 8)}</span></td>
                ...
                <td style={td}>{(() => {
                  ...
                  if (ds.length === 0) {
                    ...
                    return r.authorization_doc_url ? <span style={{ color: "#8A93A0", fontSize: "0.6875rem" }}>legacy</span> : "-";
                  }
                  return <span style={{ display: "inline-flex", flexDirection: "column", gap: 2 }}>{ds.map((d) => (
                    <a key={d.id} href={`/api/documents/${d.id}/download`} style={{ color: "#2E5FE0", fontSize: "0.71875rem" }}>{documentLabel(d.kind)}</a>
                  ))}</span>;
                })()}</td>
