...
export default async function SignupQueue({ searchParams }: { searchParams: { key?: string } }) {
  ...
  if (!token || !url || !serviceKey) {
    return <main style={wrap}><h1 style={{ fontFamily: "Playfair Display, serif" }}>Signup requests</h1><p style={{ color: "#5B6470" }}>This internal screen is not configured. Set ADMIN_REVIEW_TOKEN and SUPABASE_SERVICE_ROLE_KEY in the server environment.</p></main>;
  }
  if (searchParams?.key !== token) {
    return <main style={wrap}><h1 style={{ fontFamily: "Playfair Display, serif" }}>Signup requests</h1><p style={{ color: "#C8412E" }}>Not authorized.</p></main>;
  }
  ...
  return (
    <main style={wrap}>
      ...
      <h1 style={{ fontFamily: "Playfair Display, serif", fontSize: 30, margin: "6px 0 4px" }}>Signup requests</h1>
          ...
          <tbody>
            ...
            {rows.map((r) => (
              <tr key={r.id} style={{ background: r.status === "new" ? "#FBF3E6" : "#fff" }}>
                <td style={td}><span style={{ fontFamily: "IBM Plex Mono, monospace", fontSize: 11 }}>{new Date(r.created_at).toISOString().slice(0, 16).replace("T", " ")}</span></td>
                ...
                <td style={td}><a href={`mailto:${r.email}`} style={{ color: "#2C557F" }}>{r.email}</a>{r.phone ? <div style={{ fontFamily: "IBM Plex Mono, monospace", fontSize: 11, color: "#5B6470" }}>{r.phone}</div> : null}</td>
