...
export default async function RentIndexPage(props: { params: Promise<{ locale: string }> }) {
 ...
 try {
  ...
  if (supabase) {
   ...
   districts = ((data ?? []) as any[]).map((r: any): DRow => {
    const asset = `${assetLabel(r.asset_type, loc)}${r.segment && r.segment !== "all" ? " PLACEHOLDERMIDDOT " + segmentLabel(r.segment, loc) : ""}`;
 ...
 // language, which is how "SAR/mPLACEHOLDERSUP2PLACEHOLDERMIDDOTyr" came to be spelled here and in the unit
  ...
  <div style={{ background: "var(--cool)" }}>
   ...
   <div style={{ maxWidth: 1360, margin: "0 auto" }}>
    ...
    <div className="row between wrap" style={{ padding: "26px 24px 20px", alignItems: "flex-end", borderBottom: "1px solid var(--silver)", background: "var(--paper)", gap: 16 }}>
     <div>
      <div className="eyebrow">{ri.eyebrow}{pub.period ? " PLACEHOLDERMIDDOT " + formatPeriod(pub.period, ar) : ""}</div>
