...
export default function ReportListing({ listingId, locale }: { listingId: string; locale: "en" | "ar" }) {
  ...
  /* ELITE-4 J3-7: the two labels wrapped nothing, so the controls were unnamed. */
  const uid = useId();
  /* ELITE-4 J3-9: on success the whole form used to unmount, dropping focus to the
     body with nothing announced. The container stays, and focus lands on the
     confirmation. */
