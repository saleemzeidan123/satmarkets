    ...
    <Link href={`/${locale}/listings/${listing.id}`} className="card group relative block overflow-hidden">
      <div className="relative h-52 overflow-hidden">
        ...
        <div className="absolute start-3 top-3 flex flex-wrap items-center gap-1.5 max-w-[calc(100%-3.25rem)]">
          <span className="rounded-md bg-white/90 px-2 py-1 text-[0.625rem] font-semibold text-charcoal ring-1 ring-black/5 backdrop-blur">{assetLabel(listing.asset_type, locale)}</span>
      ...
      <div className="p-4">
        ...
        <div className="mt-1 text-[0.8125rem] text-charcoal/65">{[place, areaFig, listing.building_grade !== "n_a" ? gradeLabel(listing.building_grade, locale) : ""].filter(Boolean).join(" · ")}</div>
