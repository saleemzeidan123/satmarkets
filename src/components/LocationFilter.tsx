...
export default function LocationFilter({ locale, locations, cities, selected, basePath, baseQs }: {
  ...
  const ar = locale === "ar";
  const t = getDictionary(locale).locationFilter;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      {selLoc && (
        <div className="row gap8 wrap" style={{ alignItems: "center" }}>
          <span className="chip on" style={{ display: "inline-flex", gap: 6, alignItems: "center" }}>
            ...
            <Link href={href(null)} aria-label={t.clearLocation} style={{ textDecoration: "none", color: "inherit" }}>✕</Link>
          </span>
        </div>
      )}
      <div style={{ position: "relative", maxWidth: 420 }}>
        <input value={q} onChange={(e) => setQ(e.target.value)}
          ...
          aria-label={t.searchLabel} className="input"
          ...
          <div className="card" style={{ position: "absolute", zIndex: 20, left: 0, right: 0, marginTop: 4, maxHeight: 280, overflowY: "auto", padding: 6, boxShadow: "var(--z2)" }}>
            {results.map((l) => (
              <Link key={l.id} href={href(l.id)} onClick={() => setQ("")} className="row between"
                ...
                <span>{nameOf(l)} <span className="muted" style={{ fontSize: 12 }}>· {cityLabelOf(l.city)}{l.kind === "development" ? t.projectSuffix : ""}</span></span>
      ...
      <div className="row gap8 wrap" style={{ alignItems: "center" }}>
        ...
        <Link href={basePath.replace("/listings", "/locations")} className="chip" style={{ textDecoration: "none", opacity: 0.85 }}>{t.fullDir}</Link>
      </div>
          ...
          <div key={k} className="row gap8 wrap" style={{ alignItems: "center" }}>
            ...
            {cityLocs.filter((l) => l.kind === k).map((l) => (
              <Link key={l.id} href={href(l.id)} className={selected === l.id ? "chip on" : "chip"} style={{ textDecoration: "none" }}>
                {nameOf(l)}{l.kind === "development" ? <span style={{ opacity: 0.6, fontSize: 10.5 }}> · {t.project}</span> : null} <span style={{ opacity: 0.55, fontFamily: "var(--mono)", fontSize: 11 }}>{l.count}</span>
              </Link>
            ))}
          </div>
        ))}
        {cityLocs.length === 0 && <span className="muted" style={{ fontSize: 12.5 }}>{t.noLocations}</span>}
      </div>
    </div>
  );
}
