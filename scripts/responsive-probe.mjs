...
</style></head><body>${frag.render(frag.copy[loc])}</body></html>`;
...
console.log("\n" + pad("loc", 5) + pad("vw", 6) + pad("content", 9) + pad("fs", 6) + pad("face", 11)
...
for (const r of rows) {
  console.log(pad(r.loc, 5) + pad(r.width, 6) + pad(r.innerW, 9) + pad(r.fs, 6) + pad(r.face, 11)
    + pad(r.docOverflow, 9) + pad(r.rowOverflow, 9) + pad(r.lines, 7)
    + pad(r.widest, 8) + pad(r.minH, 6) + r.heights.join(" / "));
}
// A fragment fails if it scrolls horizontally, or if one item is wider than the
// block it sits in, which is the failure a longer string actually introduces:
// an item that cannot fit even on a line of its own.
const bad = rows.filter((r) => r.docOverflow > 0 || r.rowOverflow > 0 || r.widest > r.innerW);
console.log(bad.length === 0
  ? `\nPASS  ${rows.length} measurements, 0 horizontal overflow, no item wider than its content box`
