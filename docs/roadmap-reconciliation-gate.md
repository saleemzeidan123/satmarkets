...
| WS09 | Responsive app shell | V | "No duplicate navigation, collision, overlap or horizontal page overflow from 320 to 1920 px." | **Partial** | PKG-1B, `scripts/reflow-probe.mjs`, `scripts/responsive-probe.mjs` | Two items, both unblocked. The probe matrix stops at 1280 px and the acceptance names 1920, so the top third of the stated range has never been measured. And `main.has-tabbar` carries 62 px of bottom padding with no tab bar beneath it, currently deferred | Whether the shell is usable on a physical handset is a human question, not a probe question. The three ELITE-1 mobile seats M1 to M3 are allocated and gated on outreach authorisation |
...
| --- | --- | --- | --- |
| WS09 | Extend the responsive probe matrix from 1280 px to 1920 px, and resolve the dead 62 px `main.has-tabbar` padding | Small. One array and one rule | Yes. Physical-handset use is ELITE-1 mobile seats M1 to M3 |
