...
| 6 | P0 | وسطاء الأحياء mistranslation on Pricing | Fixed and awaiting deployment verification | Copy verified live in PKG-0A. Codex review reopened the API layer: /api/index/segments and the advisor band payloads exposed the average under a `median` key. PKG-0A.1 adds the toPublicSegment contract (payload field is `average`, no median key, no known external consumers so no alias), renames SegRow/med/md/medianLabel/band.median in the Advisor, and adds a semantic contract test. Awaiting live payload check. |
