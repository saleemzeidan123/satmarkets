...
| Production deployment | `dpl_Cp4XRmW93tbk4UetmMFZfzM114JL`, READY, target production |
| Deployment URL | `satmarkets-4djjkdxlq-sat-markets.vercel.app`. Built from `githubCommitSha` `8fed30be1dd6915e53f9dd3cf4f03d199745b7ef`, `githubCommitRef` `main`, `githubCommitVerification` `verified`. **One commit behind this row, and deliberately**, per the deployment-lag rule below: `bdc706c`, the closure addendum this row ships in, is documentation only and did not trigger its own build. `/en/notifications` and `/ar/notifications` were re-read off the production alias after the merge and confirmed live: both serve the truthful preview disclosure and the retitled "preview, not configurable" channels panel, and neither serves the removed "Mark all read", "Preferences" or switch-shaped toggle markup, in either language |
| Aliases | `satmarkets-wheat.vercel.app`, `satmarkets-sat-markets.vercel.app`, `satmarkets-git-main-sat-markets.vercel.app` |
| Commit deployed | `8fed30b`, confirmed by reading `meta.githubCommitSha` and `meta.githubCommitRef`, not `readyState` alone |
| Build ready at | Not read from the build log directly by this session; `list_deployments` reports `created` 1786198254762 (epoch ms) and `state: READY` |
| Deployment lag | The gap is one commit, the closure-addendum commit this row ships in, documentation only. Consistent with the standing rule above |
