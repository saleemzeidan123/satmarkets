...
| Item | Value |
| --- | --- |
| GitHub HEAD | `main` is `2bf652e`, "release-correction: webpack production bundler, three performance records, explicit ship branch". `next16-security` is at the same commit. The two references agree again for the first time since slice B, because `next16-security` was fast-forwarded into `main` on 4 August 2026 through the explicit promotion path. Section 1c records that promotion |
| Branch | `main`, remote `github.com/saleemzeidan123/satmarkets`. **`main` carries the whole Next.js 16 migration, and now deliberately.** How the first part of it arrived by accident is still section 1b, kept because the tool correction it produced is section 1c. Every ship from slice C onwards passes `--branch` explicitly, and the flag now has no default to omit |
| Branch position | `next16-security` at `2bf652e`, identical to `main`. Its last preview was `dpl_9gSpSvRa2w4427bv1Na8jhwr3G4p` at the slice D record; the branch has since been promoted rather than previewed further. The branch is kept rather than deleted so a rollback has a named source |
| Working tree | Clean at the time of writing, except this file, `tools/ship.py`, `tools/ship_test.py`, `tools/README.md` and `docs/handback-release-correction.md`, which this ship carries |
| Production deployment | `dpl_28FxggteRGchKrj6auC7j9BGHZCB`, READY, target production |
| Deployment URL | `satmarkets-pysps6ea5-sat-markets.vercel.app`. **One commit ahead of this row, and deliberately.** The documentation commit `d01ac7e` that carries this file built as `dpl_DagnhLPYdzr4bHAwKhx7EFXwzu1f`, `satmarkets-olhm5y6os-sat-markets.vercel.app`, READY at production, `githubCommitSha` `d01ac7ef8ba1aa13c8068eccd73ef4633f02b390` and `githubCommitRef` `main`. `/en` and `/ar/listings` were re-read off it and return 200, carry `data-dpl-id="dpl_DagnhLPYdzr4bHAwKhx7EFXwzu1f"`, carry `lang="en" dir="ltr"` and `lang="ar" dir="rtl"` respectively, carry one nonce each and a different one in each document, contain no `unpkg.com`, and carry the report-only CSP header with no enforcing header. The deployment-lag rule below is what permits this row to name the deployment before it rather than chase itself forever |
| Aliases | `satmarkets-wheat.vercel.app`, `satmarkets-sat-markets.vercel.app`, `satmarkets-git-main-sat-markets.vercel.app` |
| Commit deployed | `2bf652e`, confirmed by reading `meta.githubCommitSha` (`2bf652e3d655246f69faa3451be843da62c1b658`) and `meta.githubCommitRef` (`main`), not `readyState` alone. It is the release-correction commit and it carries all six PKG-NEXT16-SECURITY slices. The framework the production alias serves is Next.js 16.2.12, and the bundler is now webpack, confirmed in the build log by `> next build --webpack` and `▲ Next.js 16.2.12 (webpack)` rather than inferred from `package.json` |
| Build ready at | Built 17:19:05 to 17:20:33 UTC on 4 August 2026 in `iad1`, 88 seconds, compile reported at 32.4s. The previous production position was `2cfbcdc` at `dpl_DbbHaXgFsu1pqc26Ht3oySsiFZkK`, `satmarkets-cvccwo57u-sat-markets.vercel.app`, and it is what a rollback would return to. Note that rolling back to it returns to a partially migrated tree, which is the slice B state, so the rollback of choice is the previous alias promotion and not a revert of this merge |
...
| Test suite | 1759 tests, 0 failing, on `main`. The rise from 1758 is the release-correction batch, which added the assertion in `src/lib/next16Surface.test.ts` that pins the three `package.json` build scripts so the bundler line cannot revert silently. Separately from the Vitest count, `npm run ship-test` is 32 checks, up from 26, the six added by the closing-line correction in section 1c. The earlier rise from 1752 is PKG-NEXT16-SECURITY slice D: `src/lib/next16Surface.test.ts` is a new file added to the explicit list in `package.json`, holding six assertions over the async request API surface. The earlier rise from 1739 is slice C: `src/lib/rtlTextPlugin.test.ts` and `src/lib/csp.test.ts` are new files added to the explicit list in `package.json`. The earlier rise from 1679 is PKG-E1-READINESS: `src/lib/functionalTruth.test.ts`, `src/lib/search/knownQueries.test.ts`, `src/lib/authErrors.test.ts` and `src/lib/chromeGate.test.ts` are new files added to the explicit list in `package.json`, and the remainder are assertions added inside existing files |
