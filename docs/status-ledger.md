...
count is the Next.js 14 to 15 or 16 upgrade, which is an owner authorization and is listed
in section 3 with the other owner actions rather than as a finding.
...
| **Next.js 14 to 15 or 16 upgrade** | Raised by PKG-E1-READINESS slice F. Not authorised, not begun. `next@14.2.35` is the terminal 14.x release, so the 21 recorded advisories against it have no patch inside the major version and `npm audit` offers only `next@16.2.12` with `isSemVerMajor` true. The reachability of each of the 21 was checked against this application's source and written out in `docs/security-baseline.md`: 11 provably do not reach it, 10 apply by version through the App Router and middleware request path. The recommendation recorded there is yes, as its own package, before E1 exposure, because it also removes the reason `script-src` carries `'unsafe-inline'` | Ten advisories stay open with no available fix. Their residual character is denial of service and cache behaviour on a `noindex` protected preview rather than remote execution or data disclosure, which is why this is a scheduled package and not an emergency. The second cost is the Content Security Policy: nonces cannot be introduced on 14.2.35 because GHSA-ffhc-5mcf-pf4q is cross-site scripting in App Router applications that use them, so the policy's largest compromise is held in place by the framework version |
| **Vercel WAF rate-limit rule on `/api/*`** | Raised by PKG-E1-READINESS slice F. Documented, costed, not created. It is one fixed-window rule keyed on IP plus JA4 digest, configured in the Vercel dashboard, and the current plan includes it at no additional charge. Nothing in this repository can create it and nothing here should: it is account configuration, and owner ruling 7 puts it on the owner's side | Every API route is protected only by the in-process limiter, which resets with each serverless instance. Slice F deliberately did not dress that up as protection. This is the cheapest real shared limit available and the only one that costs nothing, adds no sub-processor and needs no code, so while it is open the platform has no shared limit at all. One caveat is recorded with it: the plan allows a single custom rule, and the counters are per region |
| **Upstash Redis store for `allowShared()`** | Raised by PKG-E1-READINESS slice F. Documented, costed, not installed. No account, no environment variable, no vendor contact. `src/lib/ratelimit.ts` already speaks the REST protocol and falls back cleanly, so this is configuration rather than code: `limiterIsDurable()` returns false without `KV_REST_API_URL` and `KV_REST_API_TOKEN`, and the one route that calls `allowShared` therefore runs degraded today. The free tier covers roughly 250,000 limited requests per month at two commands per check | Per-route durable limits are unavailable, and the 31 routes that import the limiter without using the shared path cannot usefully be migrated before a store exists. The decision is not only cost: a store is a new sub-processor holding client IP addresses in short-lived counter keys, which is a data-protection choice for the owner and not an engineering one |
