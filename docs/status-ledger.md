...
| **Finding 193, consent withdrawal** | Not an owner-applied migration and no migration is authored for it. It is engineering work that cannot start until an owner question is answered: how a person who posted a requirement with no account and no session proves they are that person. That question is now written up for ruling as decision O18 in `docs/decision-register.md`, with four mechanisms, the reason neither existing identifier can serve as a secret, the risks that bind whichever is chosen and the recovery path. The register row states the three dependencies | A requirement poster has no route to withdraw the consent that shares their contact details. `beef75a` corrected the label so it no longer promises a withdrawal that does not exist, which is why 192 is closed and 193 is the remaining and larger half. The obligation exists whether or not the product offers a control |
