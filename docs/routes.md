...
| /listers | public-indexable-later | none | PKG-DISCOVERY-1 item 6. In `SITEMAP_ROUTES`. Offset-paginated directory (`?page=`, `?role=owner\|broker`), 24 rows per response; the sitemap entry is the bare route, never a per-page or per-role query variant, so pagination cannot multiply the sitemap. |
