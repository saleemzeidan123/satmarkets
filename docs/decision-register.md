...
| D17 | Shared data-state pattern (WS13) | One accessible, RTL-safe, tone-driven DataState component (loading/empty/error/stale/sample/planned/permission); role=status/alert, aria-busy, tone drives icon AND colour never colour alone; server-compatible | Codex PKG-1B area 1, 2026-07-23 | src/components/DataState.tsx; adopted in listings empty state; tsc/test green. |
